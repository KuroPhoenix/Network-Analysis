...
def test_load_batch_config_applies_defaults(sample_batch_config_path: Path) -> None:
    ...
    assert config.categorization.default_category == "uncategorized"
    assert config.sampling.normalized_rates() == (1, 2, 5)
    assert config.runtime.enable_plots is True


def test_load_batch_config_rejects_invalid_category_regex(tmp_path: Path) -> None:
...
""".strip()
    ...
    with pytest.raises(BatchConfigError, match="valid regular expression"):
